...
const Tarjeta = ({ datos }) => {
  const obtenerEstiloFuerza = (rango) => {
    ...
    if (r.includes("PN") || r.includes("P.N."))
      return {
  ...
  return (
    ...
    >
      ...
      <div className="flex-1">
        ...
        <span
          className={`inline-block px-2 py-0.5 rounded-md text-[10px] font-black uppercase tracking-wider mb-2 ${estilo.bg} ${estilo.texto}`}
        >
          {datos.instRango}
        </span>
      ...
      <div className="hidden sm:block">
        <a
          ...
          className={`px-4 py-2 rounded-lg font-bold text-sm transition-colors ${estilo.bg} ${estilo.texto} hover:opacity-80`}
        >
          Llamar
        </a>
      </div>
